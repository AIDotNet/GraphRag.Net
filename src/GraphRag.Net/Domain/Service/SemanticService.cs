...
public class SemanticService(Kernel _kernel) : ISemanticService, IScopeDependency
{
    public async Task<GraphModel> CreateGraphAsync(string input)
    {
        var retryPolicy = Policy.Handle<Exception>().RetryAsync(GraphSysOption.RetryCounnt,
            (ex, count) => { Console.WriteLine($"CreateGraphAsync失败，重试{count}次，异常信息{ex.Message}"); });
        var result = await retryPolicy.ExecuteAsync<GraphModel>(async () =>
        {
            ...
            var createFun = _kernel.Plugins.GetFunction("graph", "create");
            ...
            var json = skresult.GetValue<string>()?.Trim() ?? "";
    ...
    {
        var createFun = _kernel.Plugins.GetFunction("graph", "search");
        ...
        var result = skresult.GetValue<string>()?.Trim() ?? "";
        return result;
    }
    ...
    public async IAsyncEnumerable<StreamingKernelContent> GetGraphAnswerStreamAsync(string graph, string input)
    {
        ...
        await foreach (var content in skresult) yield return content;
    }


    public async Task<string> GetGraphCommunityAnswerAsync(string graph, string community, string global, string input)
    {
        var createFun = _kernel.Plugins.GetFunction("graph", "community_search");
        var args = new KernelArguments
        {
            ["graph"] = graph,
            ["community"] = community,
    ...
    public async IAsyncEnumerable<StreamingKernelContent> GetGraphCommunityAnswerStreamAsync(string graph,
        string community, string global, string input)
    {
        var createFun = _kernel.Plugins.GetFunction("graph", "community_search");
        var args = new KernelArguments
        {
    ...
    {
        var retryPolicy = Policy.Handle<Exception>().RetryAsync(GraphSysOption.RetryCounnt,
            (ex, count) => { Console.WriteLine($"GetRelationship失败，重试{count}次，异常信息{ex.Message}"); });
        var result = await retryPolicy.ExecuteAsync<RelationShipModel>(async () =>
        {
            ...
            var createFun = _kernel.Plugins.GetFunction("graph", "relationship");
            var args = new KernelArguments(settings)
            {
                ["node1"] = node1,
                ["node2"] = node2
    ...
    {
        var createFun = _kernel.Plugins.GetFunction("graph", "mergedesc");
        var args = new KernelArguments
        {
            ["desc1"] = desc1,
            ["desc2"] = desc2
    ...
    {
        var createFun = _kernel.Plugins.GetFunction("graph", "community_summaries");
        var args = new KernelArguments
        {
            ["nodes"] = nodes
        };
    ...
    {
        var createFun = _kernel.Plugins.GetFunction("graph", "global_summaries");
        var args = new KernelArguments
        {
            ["community"] = community
        };
    ...
    public async Task<SemanticTextMemory> GetTextMemory()
    {
        ...
        switch (GraphDBConnectionOption.DbType)
        {
            ...
            case "PostgreSQL":
                ...
                memoryStore = new PostgresMemoryStore(dataSource, 1536);
                break;
        }
        ...
        var embeddingGenerator = new OpenAITextEmbeddingGenerationService(GraphOpenAIOption.EmbeddingModel,
            GraphOpenAIOption.Key, httpClient: new HttpClient(handler));
