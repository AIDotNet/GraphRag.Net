...
internal class OpenAIHttpClientHandler : HttpClientHandler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var match = regex.Match(GraphOpenAIOption.EndPoint);

        var mediaType = request.Content.Headers.ContentType.MediaType;
        var requestBody = (await request.Content.ReadAsStringAsync()).Unescape();
        ...
        if (match.Success)
        {
            var xieyi = match.Groups[1].Value;
            var host = match.Groups[2].Value;
            var port = match.Groups[3].Value; // 可选的端口号
            var route = match.Groups[4].Value;
            ...
            switch (request.RequestUri.LocalPath)
            {
                case "/v1/chat/completions":
                    ...
                    if (port.ConvertToInt32() != 0) uriBuilder.Port = port.ConvertToInt32();
